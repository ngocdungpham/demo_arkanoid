...
/**
 * Displays a compact You Win screen with a horizontal sliding animation effect.
 ...
 *
 * Animation Sequence:
 * 1. Two green bands slide in from left/right to meet in center (0.5s)
 * 2. "YOU WIN" text fades in (0.3s)
 ...
 * - Semi-transparent green band background
 * - Large, stylized "YOU WIN" text with drop shadow
 * - Automatic timeout with callback execution
 * - Non-blocking animation with proper cleanup
 *
 * Visual Design:
 * - Green semi-transparent band (height: 120px)
 * - White Orbitron font with green drop shadow
...
public class YouWinView {
    ...
    /** Background color for the band (green, semi-transparent) */
    private static final Color BAND_COLOR = Color.web("#00AA00").deriveColor(0, 1, 1, 0.5);

    /**
     * Callback interface for handling You Win screen completion.
     * Implement onExit() to handle navigation back to menu or other actions.
     */
    public interface Callbacks {
        /** Called when the You Win animation completes and screen should exit */
        void onExit();
    }

    /**
     * Constructs a YouWinView with the specified callback handler.
     ...
     */
    public YouWinView(Callbacks cb) {
        this.callbacks = cb;
        ...
        title.setEffect(new DropShadow(30, Color.web("#00FF00")));
    ...
    /**
     * Shows the You Win screen with sliding animation.
    ...
    /**
     * Hides the You Win screen with reverse sliding animation and triggers exit.
    ...
    /**
     * Gets the root node for this You Win view.
     * Add this to your scene graph to display the You Win screen.
     *
     * @return the root StackPane containing all You Win UI elements
     */
    public Node getView() {
        return root;
    }
