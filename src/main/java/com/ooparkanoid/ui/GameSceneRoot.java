...
//            }
//        });
//        gameOverView = new GameOverView(new GameOverView.Callbacks() { // <--- KHỞI TẠO NÀY
//            @Override public void onExit() {
//                // Hành động quay về Menu (được gọi sau khi animation Game Over kết thúc)
//                gameLoop.stop(); // Đảm bảo game loop đã dừng
//                SoundManager.getInstance().stopMusic();
//                onExitToMenuCallback.run(); // Gọi hàm quay về menu
//            }
//        });
//
//// Đặt overlay lên trên cùng (root của layeredScene là StackPane)
//        layeredScene.root().getChildren().add(pauseView.getView());
//        layeredScene.root().getChildren().add(gameOverView.getView());
//        // buildMenuOverlay();
//        setupStateListeners();
//        setupInputHandlers();
...
////            }
//            if (code == KeyCode.ENTER) {
//                if (stateManager.getCurrentState() == GameState.MENU) { // CHỈ GIỮ LẠI MENU
//                    if (currentMode.get() == GameMode.LOCAL_BATTLE) {
//                        startBattleMode();
//                    } else {
//                        startAdventureMode();
//                    }
//                }
//                // Bỏ qua phím ENTER khi Game Over
//                return;
//            }
