...
//    }
//}
// File: src/main/java/com/ooparkanoid/core/engine/GameManager.java (Fixed)
package com.ooparkanoid.core.engine;
...
public class GameManager implements CollisionHandler.GameFlowCallbacks {
    ...
    public void loseLife() {
        ...
        // FIX: Đọc điểm và mạng hiện tại từ stateManager
        int currentScore = stateManager.getScore();
        int currentLives = stateManager.getLives();

        currentLives--; // Giảm mạng

        effectManager.clearAll();
        powerUps.clear();
        SoundManager.getInstance().play("lose_life");

        // FIX: Cập nhật stateManager với điểm (không đổi) và mạng (đã giảm)
        stateManager.updateStats(currentScore, currentLives);
    ...
    private void recordHighScore() { recordHighScore(currentLevel); }
    ...
    public int getScore() { return stateManager.getScore(); }
    public int getLives() { return stateManager.getLives(); }
